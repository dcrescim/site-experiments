...
async function main() {
    ...
    for(var entityType of entityTypes){
        ...
        const entitiesToCreate = delta.getEntitiesToCreate(entities, existingEntities)
        const entitiesToUpdate = delta.getEntitiesToUpdate(entities, existingEntities)
        const entitiesToDelete = delta.getEntitiesToDelete(entities, existingEntities)
...
async function getEntities(entityType){
    ...
    if(entityType === 'experiments'){
        ...
        const featureIdsByName = getIdsByName(existingFeatures)
        entities = addFeatureIdToExperiments(entities, featureIdsByName)
...
async function getExistingEntities(entityType){
    ...
    if(entityType === 'features'){
        for(var existingEntity of existingEntities){
            existingEntity.variables = existingEntity.variables.filter(variable => variable.archived === false)
        }
    }
...
function addFeatureIdToExperiments(experiments, featureIdsByName){
    for(experiment of experiments){
        experiment.feature_id = featureIdsByName[experiment.feature_key]
    }
    return experiments
}
